...
class Day17Suite extends munit.FunSuite:
  ...
  test("Day17 part1 program examples"):
    assertEquals(exec(Vector(0, 2))(Computer(10, 0, 0)).a, 2L)
    assertEquals(exec(Vector(6, 2))(Computer(10, 0, 0)).b, 2L)
    assertEquals(exec(Vector(7, 2))(Computer(10, 0, 0)).c, 2L)
    assertEquals(exec(Vector(2, 6))(Computer(0, 0, 9)).b, 1L)
    assertEquals(exec(Vector(5, 0, 5, 1, 5, 4))(Computer(10, 0, 0)).output, List(0, 1, 2))
    assertEquals(exec(Vector(1, 7))(Computer(0, 29, 0)).b, 26L)
    assertEquals(exec(Vector(4, 0))(Computer(0, 2024, 43690)).b, 44354L)
    val result = exec(Vector(0, 1, 5, 4, 3, 0))(Computer(2024, 0, 0))
    assertEquals(result.a, 0L)
